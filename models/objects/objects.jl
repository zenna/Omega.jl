...
d(s1::Sphere, s2::Sphere) = norm(s1.center - s2.center)

"Distance between surfance color"
cold(s1::Sphere, s2::Sphere) = norm(s1.surface_color - s2.surface_color)
...
distinguishedcolor(sc::Sphere) = same(pairwise(cold, sc))

function main()
  scene = iid(scene_)     # Random Variable of scenes
  img = render(scene)     # Random Variable over images
  # img_ = rand(img)
  samples = rand(scene, nointersect(scene), HMCFAST)
end