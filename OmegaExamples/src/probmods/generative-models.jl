...
The key idea is that in Omega you can express stochastic processes and not just deterministic functions (like _and_).
"""

# ╔═╡ 0ce18f90-f2ef-490a-a7c4-40a6ccfa214a
md"""
A basic object in Omega is a random variable class, from which you can generate random variables. For instance, a random variable that is of Bernoulli distribution is generated by -
"""
...
md"Here, `Bernoulli()` is a random class and to get a member (a random variable, which can be sampled from) of the class we use `~`. So, `0 ~ Bernoulli()` is the `0`th member of Bernoulli class."
...
A class in Omega is actually just function of the form f(id, ω). Of course, you can specify your own classes simply by constructing a function.
"""
