...
bernoulli(p::MaybeRV{T}, ωid::Id = uid()) where T <: AbstractFloat = RandVar{Float64, true}(bernoulli, (p,), ωid)
...
inversegamma(α::MaybeRV{T}, θ::MaybeRV{T}, ωid::Id = uid()) where T <: Real =
...
# FIXME: Type
dirichlet(α::MaybeRV{T}, ωid::Id = ωid::Id=uid()) where T = RandVar{T, true}(dirichlet, (α,), ωid)

"Rademacher distribution"
rademacher(p::MaybeRV{T}, ωid::Id = uid()) where T = bernoulli(p, ωid::Id) * 2.0 - 1.0

"Categorical distribution with probability weight vector `p`"
categorical(ω::Ω, p::Vector) = quantile(Categorical(p), rand(ω))
categorical(p::MaybeRV{Vector{T}}, ωid::Id = uid()) where T <: Real = RandVar{Int, true}(categorical, (p,), ωid)
...
poisson(λ::MaybeRV{T}, ωid::Id = uid()) where T <: Real = RandVar{Int, true}(poisson, (λ,), ωid)

"Uniform distribution with lower bound `a` and upper bound `b`"
uniform(ω::Ω, a::T, b::T) where T = rand(ω) * (b - a) + a
uniform(a::MaybeRV{T}, b::MaybeRV{T}, ωid::Id=uid()) where T <: AbstractFloat =
  RandVar{T, true}(uniform, (a, b), ωid)
uniform(ω::Ω, a, b, sz::Dims) = p = rand(ω, sz) .* (b .- a) .+ a
uniform(a::MaybeRV{T}, b::MaybeRV{T}, ωid::Id = uid()) where T =
  RandVar{T, true}(uniform, (a, b), ωid)
uniform(a::MaybeRV{T}, b::MaybeRV{T}, dims::MaybeRV{Dims{N}}, ωid::Id = uid()) where {N, T} =
  RandVar{Array{T, N}, true}(uniform, (a, b, dims), ωid)

"Uniform sample from vector"
uniform(ω::Ω, a::T) where T = rand(ω, a)
uniform(a::MaybeRV{T}, ωid::Id=uid()) where {V, T <: Vector{V}} =
  RandVar{V, true}(uniform, (a,), ωid)

"Discrete uniform distribution with range `range`"
uniform(range::UnitRange{T}, ωid=uid()) where T =
...
normal(μ::MaybeRV{T}, σ::MaybeRV{T}, ωid::Id = uid()) where T <: AbstractFloat =
  RandVar{T, true}(normal, (μ, σ), ωid)
normal(μ::MaybeRV{T}, σ::MaybeRV{T}, dims::MaybeRV{Dims{N}}, ωid::Id = uid()) where {N, T <: AbstractFloat} =
  RandVar{Array{T, N}, true}(normal, (μ, σ, dims), ωid)
# normal(ω::Ω, μ, σ) = rand(ω, Normal(μ, σ))
# normal(ω::Ω, μ, σ, ωid::Id = uid()) = normal(parent(ω)[ωid], μ, σ)
...
mvnormal(μ::MaybeRV{T1}, Σ::MaybeRV{T2}, ωid::Id = uid()) where {T1, T2} =
...
logistic(μ::MaybeRV{T}, s::MaybeRV{T}, ωid::Id = uid()) where T =
  RandVar{T, true}(logistic, (μ, s), ωid)
logistic(μ::MaybeRV{T}, s::MaybeRV{T}, dims::MaybeRV{Dims{N}}, ωid::Id = uid()) where {N, T} =
...
exponential(λ::MaybeRV{T}, ωid::Id = uid()) where T = RandVar{T, true}(exponential, (λ,), ωid)
exponential(λ::MaybeRV{T}, dims::MaybeRV{Dims{N}}, ωid::Id = uid()) where {N, T} = RandVar{T, true}(exponential, (λ, dims), ωid)
...
kumaraswamy(a::MaybeRV{T}, b::MaybeRV{T}, ωid::Id = uid()) where T =
  RandVar{T, true}(kumaraswamy, (a, b), ωid)
kumaraswamy(a::MaybeRV{T}, b::MaybeRV{T}, dims::MaybeRV{Dims{N}}, ωid::Id = uid()) where {N, T} =
