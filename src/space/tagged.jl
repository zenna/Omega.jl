...
@inline Base.rand(tω::TaggedΩ, ::Type{T}, dim::Dims, dims::Integer...) where T= rand(tω.taggedω, T, dim, dims...; rng = rng(tω))

@inline Base.rand(tω::TaggedΩ, arr::Array) = rand(tω.taggedω, arr; rng = rng(tω))
@inline Base.rand(tω::TaggedΩ, arr::Array, dims::Dims) = rand(tω.taggedω, arr, dims; rng = rng(tω))
@inline Base.rand(tω::TaggedΩ, arr::Array, dim::Dims, dims::Integer...) = rand(tω.taggedω, arr, dim, dims...; rng = rng(tω))
...
@inline Base.rand(tω::TaggedΩ, ur::UnitRange, dim::Dims, dims::Integer...) = rand(tω.taggedω, ur, dim, dims...; rng = rng(tω))
