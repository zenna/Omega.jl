...
"Sample `n` from `x`"
function Base.rand(x::RandVar, n::Integer; alg::Algorithm = defalg(x), ω = defΩ(alg)(), kwargs...)
  rand(x, n, alg, ω; kwargs...)
end

"Sample 1 from `x`"
function Base.rand(x::RandVar; alg::Algorithm = defalg(x), ω = defΩ(alg)(), kwargs...)
  first(rand(x, 1, alg, ω; kwargs...))
end
